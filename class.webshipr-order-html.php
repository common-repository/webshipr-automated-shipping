...
if ( ! class_exists( 'WebshiprOrderHtml' ) ) {
	...
	class WebshiprOrderHtml{
		...
		public function __construct(WC_Order $wooOrder){
			...
			$this->postType = get_post_type($wooOrder->get_id());
            ...
            if($this->api->CheckConnection()){
                ...
                $this->orderExists = $this->api->OrderExists($wooOrder->get_id());

                if($this->orderExists){
                    $this->webshiprOrder = $this->api->GetOrder($wooOrder->get_id());
                }else{
